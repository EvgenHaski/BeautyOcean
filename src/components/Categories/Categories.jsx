...
const Categories = () => {
  ...
  const suggestions = React.useMemo(() => {
    ...
    setCursor(-1);
    scrollIntoView(0);
    return categoriesList.filter((item) => item.name.toLowerCase().includes(search.toLowerCase()));
  }, [search]);
  // При клике не на таргет, исчезает окно
  React.useEffect(() => {
    const handleClickOutside = (event) => {
      if (!event.composedPath || !event.composedPath().includes(searchContainerRef.current)) {
        setIsOpen(false);
      }
    };
  ...
  const keyboardNavigation = (e) => {
    if (e.key === 'ArrowDown') {
      isOpen ? setCursor((c) => (suggestions.length - 1 ? c + 1 : c)) : setIsOpen(true);
            ...
            <div className={styles.li_wrapper}>
              {suggestions.map((obj, idx) => (
                <li
                  ...
                  className={(cursor === idx ? true : false) ? styles.active : ``}
                  key={obj.name}>
                  {obj.name}
                </li>
              ))}
